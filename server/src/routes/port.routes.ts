...
import { mainDb, shipmentDb } from '../db';
                ...
                AND: [
                    { isActive: true },
                    {
                        OR: [
                            ...
                            { countryName: { contains: searchTerm } }
                        ]
                    }
                ]
            },
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        // Check if port is used in any shipments (cross-database check)
        const shipmentsUsingPort = await shipmentDb.shipment.count({
            where: {
                OR: [
                    { polId: req.params.id },
                    { podId: req.params.id }
